...
add_action('admin_head-toplevel_page_kumihimo', function () {
...
add_filter('nhp-opts-saved-text-kumihimo', function ($default) {
...
add_action('nhp-opts-options-validate-kumihimo', function () {
	if (isset($_POST['wipe']) && $_POST['wipe']) {
		try {
			Indexer::clear();
			set_transient('es-wiped', 1, 30);
		} catch (\Exception $ex) {
			$errors = get_transient('nhp-opts-errors-kumihimo');
			$errors[] = array(
				'section_id' => 'index'
			);

			set_transient('nhp-opts-errors-kumihimo', $errors, 1000);
