...
class FacetingOptionsWidget extends \WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		if ($async) {
			...
			wp_enqueue_script('kumihimo', plugins_url('/js/ajax.js', __FILE__), array('jquery'));

			kumihimo\Theme::setSelector($instance['asyncReplace']);

			wp_localize_script('kumihimo', 'esfaceting', array(
				'replace' => $instance['asyncReplace'],
				'offset' => $offset
			));
		}

		$facets = kumihimo\Faceting::all();
					...
					foreach ($facet['available'] as $option) {
						if ($option['count'] != $wp_query->found_posts) {
							...
							$prep[$type]['avail'][] = array(
								'url' => kumihimo\Faceting::urlAdd($url, $type, $option['slug']),
								'option' => $option
							);
						}
					}
